...
const mobileView = mql.matches*/ //PRUEBA DE MATCHES PARA SEPARAR MOBILE DE DESKTOP
...
function Home() {
  ...
  return (
      <div className="w-full min-h-screen">
          <div className='w-full min-h-screen block bg-gray-900 quini:hidden'>
            {posts.map((tiktok)=>{
              return (
                <div className='snap-start overflow-hidden' key={tiktok.id}>
                  <VideoPlayer {...tiktok} isAuth={isAuth} setIsAuth={setIsAuth} key={tiktok.id}/>
                </div>
              )
            })}
          </div>
          ...
          <Notifications isAuth={isAuth} setIsAuth={setIsAuth} displayNotifications={displayNotifications} setNotifications={setNotifications} notifications={notifications}  posts={posts} />
          <NavbarBottom  addTik={addTik} newTiktok={newTiktok} displayNotifications={displayNotifications} notifications={notifications} isAuth={isAuth} setIsAuth={setIsAuth} />
          ...
          <div className='hidden mt-[60px] w-full quini:flex min-h-screen'>
            ...
            <div className='w-full min-h-screen flex flex-col laptop:items-center'>
              {posts.map((tiktokWeb)=>{
                return (
                  <>
                    <ContentFeed {...tiktokWeb} isAuth={isAuth} setIsAuth={setIsAuth} key={tiktokWeb.id, tiktokWeb.timestamp}/>
                  </>
                )
              })}
            </div>
